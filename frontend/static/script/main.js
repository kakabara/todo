...
ApiServer.getTasks = () => { return sendRequest('http://127.0.0.1:5000/api/v1/tasks', 'GET', null); };
ApiServer.deleteTask = (task_id) => { return sendRequest('http://127.0.0.1:5000/api/v1/tasks/' + task_id, 'DELETE', null); };
ApiServer.editTask = (body) => { return sendRequest('http://127.0.0.1:5000/api/v1/tasks/' + body['id'], 'PUT', body); };
ApiServer.createTask = (body) => { return sendRequest('http://127.0.0.1:5000/api/v1/tasks/', 'POST', body); };
class Task {
    constructor(task) {
...
class View {
    ...
    render_task(task) {
        let htmlTask = document.getElementById(task['id'].toString());
        if (htmlTask) {
            ...
            statusesTasks.forEach((status) => { if (classList.indexOf(status) >= 0)
                taskLabel.classList.toggle(status); });
            ...
            let tmp_task = Object.assign({}, task);
